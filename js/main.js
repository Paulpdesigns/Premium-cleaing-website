...
function initShell() {
  ...
  const onScroll = () => {
    ...
    $(".scroll-progress")?.style.setProperty("--progress", `${(window.scrollY / (document.documentElement.scrollHeight - innerHeight)) * 100}%`);
    $(".back-top")?.classList.toggle("is-visible", window.scrollY > 650);
    $(".mobile-contact-bar")?.classList.toggle("is-visible", window.scrollY > innerHeight * 0.75);
  };
  onScroll();
